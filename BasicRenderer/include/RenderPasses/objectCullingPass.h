...
public:
	...
	PassReturn Execute(PassExecutionContext& context) override {
		...
		contextRef.indirectCommandBufferManager->ForEachIndirectBuffer([&](uint64_t view,
...

			{
				...
				miscRootConstants[ACTIVE_DRAW_SET_INDICES_BUFFER_SRV_DESCRIPTOR_INDEX] = contextRef.objectManager->GetActiveDrawSetIndices(flags)->GetSRVInfo(0).slot.index;
