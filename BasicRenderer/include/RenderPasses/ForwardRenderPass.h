...
public:
	...
	std::vector<ID3D12GraphicsCommandList*> Execute(RenderContext& context) override {
		...
		D3D12_GPU_VIRTUAL_ADDRESS objectBufferAddress = context.currentScene->GetObjectManager()->GetPerObjectBuffers()->GetBuffer()->GetAPIResource(context.frameIndex)->GetGPUVirtualAddress();
