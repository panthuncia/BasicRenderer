...
public:
    ...
    PassReturn Execute(PassExecutionContext& context) override {
		...
		auto& cl = contextRef.commandList;



        cl.SetDescriptorHeaps(contextRef.textureDescriptorHeap.GetHandle(), contextRef.samplerDescriptorHeap.GetHandle());
...

        {
...

            {
                ...
                const uint32_t dstUavIndex =
                    ...
                    j.dstPrefilteredCubemap->GetUAVShaderVisibleInfo(mip, 0).slot.index;
