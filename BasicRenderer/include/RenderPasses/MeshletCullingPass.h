...
public:
	...
	PassReturn Execute(PassExecutionContext& context) override {
		...
		unsigned int numDraws = contextRef.drawStats.numDrawsInScene;
		...
		unsigned int cameraIndex = contextRef.viewManager->Get(contextRef.currentScene->GetPrimaryCamera().get<Components::RenderViewRef>().viewID)->gpu.cameraBufferIndex;
		...
		if (getShadowsEnabled()) {
			...
			lightQuery.each([&](flecs::entity e, Components::Light light, Components::LightViewInfo& lightViewInfo, Components::DepthMap lightDepth) {
				...
				for (auto& viewID : lightViewInfo.viewIDs) {
					...
					auto view = contextRef.viewManager->Get(viewID);
