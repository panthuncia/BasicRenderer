...
public:
	...
	PassReturn Execute(PassExecutionContext& context) override {
		...
		colorAttachment.rtv = { contextRef.rtvHeap.GetHandle(), contextRef.frameIndex };
		...
		passInfo.width = contextRef.outputResolution.x;

		passInfo.height = contextRef.outputResolution.y;
