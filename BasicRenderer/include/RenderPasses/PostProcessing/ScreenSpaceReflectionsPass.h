...
public:
    ...
    PassReturn Execute(PassExecutionContext& context) override {
        ...
        //contextRef.commandList.ClearRenderTargetView( m_pSSSROutput->GetRTVInfo(0).slot, m_pSSSROutput->GetClearColor());



		contextRef.commandList.SetDescriptorHeaps(contextRef.textureDescriptorHeap.GetHandle(), contextRef.samplerDescriptorHeap.GetHandle());
  ...
  //          D3D12_RESOURCE_STATE_UNORDERED_ACCESS
		...
		//contextRef.commandList->ResourceBarrier(1, &barrier);



        // Clear as UAV

  //      contextRef.commandList.ClearUavFloat(
