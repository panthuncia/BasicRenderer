...
public:
    ...
    PassReturn Execute(PassExecutionContext& context) override {
        ...
        commandList.Dispatch((contextRef.renderResolution.x + (XE_GTAO_NUMTHREADS_X*2)-1) / (XE_GTAO_NUMTHREADS_X*2), (contextRef.renderResolution.y + XE_GTAO_NUMTHREADS_Y-1) / XE_GTAO_NUMTHREADS_Y, 1 );
