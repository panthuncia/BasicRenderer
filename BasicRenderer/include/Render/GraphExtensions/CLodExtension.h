...
private:
    ...
    public:
        ...
        PassReturn Execute(PassExecutionContext& context) override {
            ...
            contextRef.viewManager->ForEachFiltered(filter, [&](uint64_t view) {

                auto viewInfo = contextRef.viewManager->Get(view);

                auto cameraBufferIndex = viewInfo->gpu.cameraBufferIndex;

                auto workloads = contextRef.indirectCommandBufferManager->GetViewIndirectBuffersForRenderPhase(view, m_renderPhase);
                ...
                for (auto& wl : workloads) {
                    ...
                    record.activeDrawSetIndicesSRVIndex = contextRef.objectManager->GetActiveDrawSetIndices(wl.flags)->GetSRVInfo(0).slot.index;
            ...
            uintRootConstants[CLOD_NUM_RASTER_BUCKETS] = contextRef.materialManager->GetRasterBucketCount();
    ...
    public:
        ...
        PassReturn Execute(PassExecutionContext& context) override {
            ...
            auto numBuckets = contextRef.materialManager->GetRasterBucketCount();
    ...
    public:
        ...
        PassReturn Execute(PassExecutionContext& context) override {
            ...
            contextRef.commandList.BeginPass(p);
            ...
            for (uint32_t i = 0; i < numBuckets; ++i) { // TODO: Compaction for zero-count buckets?
                ...
                auto flags = contextRef.materialManager->GetRasterFlagsForBucket(i);
