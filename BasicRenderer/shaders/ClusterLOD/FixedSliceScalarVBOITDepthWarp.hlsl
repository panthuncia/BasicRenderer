...
bool IsVirtualSliceOccupied(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config, uint sliceIndex)
{
    return sliceIndex < config.virtualSliceCount && occupancyHistogramBuffer[sliceIndex] != 0u;
}

uint CountOccupiedVirtualSlices(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config)
{
...
uint GetOccupiedRankAtOrBefore(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config, uint sliceIndex, out bool found)
{
    found = false;
...
uint GetOccupiedRankAtOrAfter(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config, uint sliceIndex, out bool found)
{
    found = false;
    uint occupiedRank = 0u;
    [unroll(CLOD_FIXED_SLICE_SCALAR_VBOIT_DEFAULT_VIRTUAL_SLICE_COUNT)]
...
uint GetNextOccupiedSliceIndex(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config, uint sliceIndex, out bool found)
{
    found = false;
    [unroll(CLOD_FIXED_SLICE_SCALAR_VBOIT_DEFAULT_VIRTUAL_SLICE_COUNT)]
    for (uint prefixIndex = 0u; prefixIndex < CLOD_FIXED_SLICE_SCALAR_VBOIT_DEFAULT_VIRTUAL_SLICE_COUNT; ++prefixIndex)
...
uint GetPreviousOccupiedSliceIndex(StructuredBuffer<uint> occupancyHistogramBuffer, CLodFixedSliceScalarVBOITConfig config, uint sliceIndex, out bool found)
{
    found = false;
...
float ComputeLinearFallbackWarpedCoordinate(CLodFixedSliceScalarVBOITConfig config, float baseSliceCoordinate)
{
...
[shader("compute")]
[numthreads(64, 1, 1)]
void CLodFixedSliceScalarVBOITDepthWarpCS(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    if (lutIndex >= CLOD_FIXED_SLICE_SCALAR_VBOIT_DEPTH_WARP_LUT_RESOLUTION)
    {
        return;
    }
    ...
    const float baseSliceCoordinate = config.virtualSliceCount <= 1u
        ? 0.0f
        : ((float)lutIndex / (float)(CLOD_FIXED_SLICE_SCALAR_VBOIT_DEPTH_WARP_LUT_RESOLUTION - 1u)) *
            (float)(config.virtualSliceCount - 1u);
    depthWarpEntry.warpedSliceCoordinate = ComputeWarpedSliceCoordinateFromHistogram(
        occupancyHistogramBuffer,
        config,
        baseSliceCoordinate);
    if (lutIndex + 1u < CLOD_FIXED_SLICE_SCALAR_VBOIT_DEPTH_WARP_LUT_RESOLUTION)
    {
        const float nextBaseSliceCoordinate = config.virtualSliceCount <= 1u
            ? 0.0f
            : ((float)(lutIndex + 1u) / (float)(CLOD_FIXED_SLICE_SCALAR_VBOIT_DEPTH_WARP_LUT_RESOLUTION - 1u)) *
                (float)(config.virtualSliceCount - 1u);
