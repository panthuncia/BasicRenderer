...
void ReyesDiceCS(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    if (CLOD_REYES_DICE_QUEUE_READ_OFFSET_DESCRIPTOR_INDEX != 0xFFFFFFFFu)
    {
        StructuredBuffer<uint> readOffsetBuffer = ResourceDescriptorHeap[CLOD_REYES_DICE_QUEUE_READ_OFFSET_DESCRIPTOR_INDEX];
